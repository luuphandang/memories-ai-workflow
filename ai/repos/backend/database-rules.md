...
- Schema ownership: không đặt table nghiệp vụ hoặc table hạ tầng trong `public`; mỗi bounded context/platform concern sở hữu schema riêng và entity phải khai báo cùng schema với migration. Hiện tại media dùng `media.assets`, transactional outbox cho worker dùng `background_jobs.outbox_messages`; tên index/constraint có prefix schema để tránh va chạm.
